...
{
protected:
  int size; //размер мультистека
  T* Mas; //указатель на область памяти под хранение мультистека
  int n; //количество стеков в мультистеке
  TNewStack<T>** h; //массив указателей на начало каждого стека в мультистеке
  int GetFreeMem(); //общее количество свободных ячеек в каждом стеке
  void Repack(int _n); //перепаковка стека с номером _n
public:
  TMStack(int _size, int _n); //конструктор инициализатор с количеством стеков _n и его размером _size
  TMStack(TMStack<T> &A);
  void Put(int _n, const  T &A); //положить в стек номер _n значение A
  T Get(int _n); //взять из стека с номером _n элемент
...
TMStack<T> ::TMStack(TMStack<T> &A)
{
  ...
  Mas = new T[size];
  h = new TNewStack<T>*[n];
  Mas = A.Mas;//копируем данные общего массива
  int* p = new int[n];
  for (int i = 0; i < n; i++)
    p[i] = A.h[i]->GetSize();
...
void TMStack<T>::Repack(int k)
{
  ...
  T** new_start = new T*[n];
  for (int i = 0; i < n; i++)
    newSize[i] = add_every + h[i]->TNewStack<T>::GetTop() + 1;
  newSize[k] += add_full;
  ...
  for (int i = 0; i < n; i++)
  {
    if (new_start[i] <= old_start[i])
      for (int j = 0; j < h[i]->TNewStack<T>::GetTop() + 1; j++)
    ...
    else
    {
      ...
      for (; s < n; s++)
        if (new_start[s] <= old_start[s])
          break;
      for (int j = s - 1; j >= i; j--)
        for (int r = h[i]->TNewStack<T>::GetTop(); r >= 0; r--)
